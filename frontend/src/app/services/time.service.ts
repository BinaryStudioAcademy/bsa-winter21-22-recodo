...
export class TimeService {
  ...
  public calculateTimeDifference(oldDate: Date) {
    ...
    const DAY = 24 * HOUR;
    const MONTH = 30 * DAY;
    var timeSpan = (new Date().getTime() - new Date(oldDate).getTime())/1000;
    var delta = Math.floor(timeSpan);
    ...
    if (delta < 12 * MONTH)
    {
      var months = Math.floor(timeSpan / DAY / 30);
      return months <= 1 ? "one month ago" : months + " months ago";
    }
    else
    {
      var years = Math.floor(timeSpan / DAY / 365);
